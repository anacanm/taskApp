...
router.get('/users', (req, res) => {     //sending a get request to the users endpoint will return all users from the database
...
router.get('/users/id/:id', (req, res) => {    //gets user by id, :id acts as a dynamic route
...
router.post('/users', (req, res) => {    //when a user posts information to the server
...
router.patch('/users/update/:id', async (req, res) => {     // sets document specified by the id to req.body (what data is sent to the endpoint)
    try {
...
router.delete('/users/delete/:id', async (req, res) => { //deletes user by id
    try {
